...
class ScheduleNetManager {
    ...
    func update(schedule: Schedule, completion: @escaping ()->()) {
        
        guard let url = URL(string: "\(Bundle.main.REST_API_URL)/api/schedule/update") else { return }
    ...
    func read(sid: Int, completion: @escaping (Schedule)->()) {
        guard let url = URL(string: "\(Bundle.main.REST_API_URL)/api/schedule/read/\(sid)") else { return }
        ...
        URLSession.shared.dataTask(with: request) { data, res, err in
            ...
            if let safeData = data {
                do {
                    let decodedData = try JSONDecoder().decode(Schedule.self, from: safeData)
                    dump(decodedData)
