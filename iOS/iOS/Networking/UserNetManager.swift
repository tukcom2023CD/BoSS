...
class UserNetManager {
    ...
    // 사용자 데이터 읽기
    func read(uid: Int, completion: @escaping ([User])->()) {
        guard let url = URL(string: "\(Bundle.main.REST_API_URL)/api/user/read/\(uid)") else { return }
    ...
    // 사용자 데이터 수정
    func update(user: User, completion: @escaping ()->()) {
        
        guard let url = URL(string: "\(Bundle.main.REST_API_URL)/api/user/update") else { return }
